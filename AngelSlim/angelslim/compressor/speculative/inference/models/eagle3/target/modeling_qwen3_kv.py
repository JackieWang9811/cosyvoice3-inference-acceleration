...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    Args:
        ...
        unsqueeze_dim (`int`, *optional*, defaults to 1):
            ...
            be properly broadcasted to the dimensions of q and k. For example, note
        ...
        if self.config._attn_implementation != "eager":
            if self.config._attn_implementation == "sdpa" and kwargs.get(
                ...
                logger.warning_once(
                    ...
                    "eager attention. This warning can be removed using "
...
@add_start_docstrings(
    "The bare Qwen3 Model outputting raw hidden-states without any specific head on top.",  # noqa: E501
...
QWEN3_INPUTS_DOCSTRING = r"""
    Args:
        ...
        attention_mask (`torch.Tensor` of shape `(batch_size, sequence_length)`, *optional*): # noqa: E501
            ...
            `input_ids` have to be input (see `past_key_values`).
        ...
        past_key_values (`Cache`, *optional*):
            ...
            and in the cross-attention blocks) that can be used to speed up sequential
            decoding. This typically consists in the `past_key_values` returned by the
            model at a previous stage of decoding, when `use_cache=True` or
        ...
        cache_position (`torch.LongTensor` of shape `(sequence_length)`, *optional*):
            ...
            Contrarily to `position_ids`, this tensor is not affected by padding.
...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    Each layer is a [`Qwen3DecoderLayer`]
            ...
            if attention_mask is not None and past_key_values is not None:
                ...
                if is_padding_right:
                    raise ValueError(
                        ...
                        "for Flash Attention version of Qwen3. Make sure to call "
                        "`tokenizer.padding_side  = 'left'` before tokenizing "
                        "the input. "
                    )
    ...
    def _prepare_4d_causal_attention_mask_with_cache_position(
        ...
        `(batch_size, 1, query_length, key_value_length)` from a 2D mask of shape
        ...
        Args:
            attention_mask (`torch.Tensor`):
                ...
                or a 4D attention mask of shape `(batch_size, 1, query_length,
                key_value_length)`.
            sequence_length (`int`):
                The sequence length being processed.
            target_length (`int`):
                The target length: when generating with static cache, the mask
                should be as long as the static cache, to account for the 0 padding,
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        r"""
            ...
            logits_to_keep (`int` or `torch.Tensor`, *optional*):
                ...
                If `0`, calculate logits for all `input_ids` (special case). Only
                last token logits are needed for generation, and calculating them
                only for that token can save memory, which becomes pretty significant
                for long sequences or large vocabulary size. If a `torch.Tensor`,
                must be 1D corresponding to the indices to keep in the sequence
                length dimension. This is useful when using packed tensor format
                (single dimension for batch and sequence length).
...
@add_start_docstrings(
    """
    The Qwen3 Model transformer with a sequence classification head
    on top (linear layer).

    [`Qwen3ForSequenceClassification`] uses the last token in order to
    do the classification, as other causal models (e.g. GPT-2) do.
    ...
    the last token that is not a padding token in each row. If no `pad_token_id`
...
class Qwen3ForSequenceClassification(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        if self.config.pad_token_id is None:
            ...
            last_non_pad_token = -1
            logger.warning_once(
                f"{self.__class__.__name__} will not detect padding tokens in"
                f"`inputs_embeds`. Results may be unexpected if using padding "
...
@add_start_docstrings(
    """
    The Qwen3 Model transformer with a token classification head on top
...
@add_start_docstrings(
    """
The Qwen3 Model transformer with a span classification head on top for extractive
    ...
    def forward(
        ...
        **kwargs,
        ...
        start_positions (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            ...
            the token classification loss. Positions are clamped to the length of the
            sequence (`sequence_length`). Position outside of the sequence
            are not taken into account for computing the loss.
        end_positions (`torch.LongTensor` of shape `(batch_size,)`, *optional*):
            Labels for position (index) of the end of the labelled span for computing
            the token classification loss. Positions are clamped to the length of the
            sequence (`sequence_length`). Position outside of the sequence are not taken
